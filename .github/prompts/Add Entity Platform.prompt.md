...
```python
"""[Platform] platform for Sony Projector."""
...
from .[entity_file] import SonyProjector[EntityName]
...
async def async_setup_entry(
    ...
    coordinator: SonyProjectorDataUpdateCoordinator = hass.data[DOMAIN][
        entry.entry_id
    ]

    async_add_entities(
        [
            SonyProjector[EntityName](coordinator, entry),
...
```python
"""[Entity description] for Sony Projector."""
...
class SonyProjector[EntityName](
    SonyProjectorEntity,
    [PlatformEntityClass],
):
...
- [ ] Entity class inherits from both `SonyProjectorEntity` and platform class
...
- **Base entity:** `SonyProjectorEntity` in `entity/base.py`
- **Coordinator:** `SonyProjectorDataUpdateCoordinator`
