...
This document describes the technical architecture of the Sony Projector custom component for Home Assistant.
...
│   ├── __init__.py          # Exports SonyProjectorDataUpdateCoordinator
...
│   ├── __init__.py          # Exports SonyProjectorEntity
...
- `base.py` - Main coordinator class (`SonyProjectorDataUpdateCoordinator`)
...
**Key class:** `SonyProjectorDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `SonyProjectorApiClient`
...
- `SonyProjectorConfigFlowHandler` (main flow)
- `SonyProjectorOptionsFlow` (options)
...
**Key class:** `SonyProjectorEntity` (in `entity/base.py`)
...
2. `SonyProjectorEntity` for common functionality
...
1. Create directory: `custom_components/sony_projector/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `SonyProjectorEntity`
